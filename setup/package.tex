...
\usepackage[a4paper,text={146.4true mm,239.2 true mm},top= 26.2true mm,left=31.8 true mm,head=6true mm,headsep=6.5true mm,foot=16.5true mm]{geometry}
                                            % 支持版面尺寸设置
\usepackage{siunitx}                        % 支持国际标准单位（替代已废弃的 SIunits）
\usepackage{titlesec}                       % 控制标题的宏包
\usepackage{titletoc}                       % 控制目录的宏包
\usepackage{fancyhdr}                       % fancyhdr宏包 支持页眉和页脚的相关定义
\usepackage[AutoFakeBold=3,PunctStyle=hangmobanjiao]{xeCJK}
\usepackage{xcolor}                         % 支持彩色（替代已废弃的 color 宏包）
...
\usepackage{caption}                        % 现代化的标题定制宏包（替代已废弃的 ccaption）
\usepackage{subcaption}                     % 支持子图（替代已废弃的 subfigure 宏包）
\usepackage[sort&compress,numbers]{natbib}  % 支持引用缩写的宏包
\usepackage{enumitem}                       % 使用enumitem宏包,改变列表项的格式
\usepackage{calc}                           % 长度可以用+ - * / 进行计算
\usepackage{newtxtext,newtxmath}            % 现代 Times 风格字体（替代已废弃的 txfonts）
...
%支持算法环境
\usepackage{algorithm}                      % 算法浮动体环境（替代 algorithm2e 以避免包冲突）
\usepackage{algorithmic}                    % 算法伪代码排版
...
% 请使用 xelatex + bibtex + xelatex + xelatex 编译。
\usepackage[unicode,
            pdfstartview=FitH,
