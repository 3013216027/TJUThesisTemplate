...
% 跨平台字体回退：优先使用 Adobe 中文字体，不存在时回退到 TeX Live 自带的 Fandol 字体
\IfFontExistsTF{Adobe Song Std}{
  \newCJKfontfamily[song]\song{Adobe Song Std}
  \setCJKmainfont{Adobe Song Std}
}{
  \newCJKfontfamily[song]\song{FandolSong}
  \setCJKmainfont{FandolSong}     % TeX Live 自带宋体
}
\IfFontExistsTF{Adobe Fangsong Std}{
  \newCJKfontfamily[fangsong]\fs{Adobe Fangsong Std}
  \setCJKmonofont{Adobe Fangsong Std}
}{
  \newCJKfontfamily[fangsong]\fs{FandolFang} % TeX Live 自带仿宋
  \setCJKmonofont{FandolFang}     % TeX Live 自带仿宋
}
\IfFontExistsTF{Adobe Heiti Std}{
  \newCJKfontfamily[hei]\hei{Adobe Heiti Std}
  \setCJKsansfont{Adobe Heiti Std}
}{
  \newCJKfontfamily[hei]\hei{FandolHei}
  \setCJKsansfont{FandolHei}      % TeX Live 自带黑体
}
\IfFontExistsTF{Adobe Kaiti Std}{
  \newCJKfontfamily[kai]\kai{Adobe Kaiti Std}
}{
  \newCJKfontfamily[kai]\kai{FandolKai} % TeX Live 自带楷体
}
% 英文字体回退：优先使用系统字体，不存在时回退到 TeX Live 自带的 TeX Gyre 字体
\IfFontExistsTF{Times New Roman}{
  \setmainfont{Times New Roman}   % 英文衬线
}{
  \setmainfont{TeX Gyre Termes}   % TeX Live 自带，兼容 Times New Roman
}
\IfFontExistsTF{Arial}{
  \setsansfont{Arial}             % 英文无衬线
}{
  \setsansfont{TeX Gyre Heros}    % TeX Live 自带，兼容 Helvetica/Arial
}
\IfFontExistsTF{Courier New}{
  \setmonofont{Courier New}       % 英文等宽
}{
  \setmonofont{TeX Gyre Cursor}   % TeX Live 自带，兼容 Courier
}
...
% \punctstyle{hangmobanjiao}             % 调整中文字符的表示，行内占一个字符宽度，行尾占半个字符宽度 => use XeLaTeX
...
\newcommand{\distance}{\,\Vert\,}  % 距离/KL散度分隔符

% 其它宏
\newcommand{\todo}[1]{\textcolor{red}{\{TODO:#1\}}}  % \todo{} 生成红色字体的“TODO: ”，方便编写草稿时打标记。
\newcommand*{\ldash}[0]{\raisebox{0.5mm}{\,-----\,}}  % 破折号
\newlength{\wordsep}                                   % 公式注释后与正文的间距（用于 tabularx 注释表格后，如 tables.tex 中的 \vspace{\wordsep}）
\setlength{\wordsep}{\baselineskip}
...
\titlecontents{chapter}[2em]{\vspace{.5\baselineskip}\xiaosan\song}
             {\prechaptername\thecontentslabel\postchaptername\qquad}{}
...
%%%%%% 定制浮动图形和表格标题样式 %%%%%%
% 使用 caption 宏包统一设置标题格式（替代手动重定义 \@makecaption）
\DeclareCaptionFont{wuhao}{\wuhao}
\DeclareCaptionFont{songti}{\song}
\captionsetup{
  font={wuhao,songti},
  labelsep=quad,
...
\captionsetup[subfigure]{labelformat=simple}  % 子图标签格式
\captionsetup[longtable]{labelsep=quad}       % longtable 标签分隔符
...
%\cleardoublepage%
  ...
  \begin{figure}[h]
  \centering
  \includegraphics[width=0.4\textwidth]{figures/tju.pdf}
  \end{figure}
  \vspace*{21pt}
  ...
  \begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/tjulogo.pdf}
  \end{figure}
