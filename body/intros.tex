...
\section{系统要求}
     ...
     TeX Live 2020~或以上版本。我们推荐您使用最新版本的~TeX Live~发行版（\url{https://www.tug.org/texlive/}），macOS~用户也可选择~MacTeX（\url{https://www.tug.org/mactex/}）。编辑器推荐使用~VS Code~（配合~LaTeX Workshop~插件）、Sublime Text~或~TeXstudio。注意源文件必须以~UTF-8~编码保存。本模板在~Windows、macOS~和~Linux~平台下均可使用。
