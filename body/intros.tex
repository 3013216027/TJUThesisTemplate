...
\begin{enumerate}
  ...
  \item 如果在~pdf~书签中，中文显示乱码的话，则注意以下说明：
    \begin{verbatim}
     ...
     \end{verbatim}
  \item 建议采用~XeLaTeX~编译方式：
  \begin{enumerate}
    \item xelatex + bibtex + xelatex + xelatex。本模板使用~xeCJK~宏包处理中文，必须使用~XeLaTeX~引擎进行编译。图片文件推荐使用~PDF~或~PNG~格式（XeLaTeX~原生支持），也兼容~EPS~格式。
          如果您选择在命令行中操作，可以在编译的时候依次输入~xelatex tjumain, bibtex tjumain, xelatex tjumain, xelatex tjumain，编译完成之后，需要手动打开~pdf~文件。也可以直接使用~\verb|make|~命令一键完成以上编译步骤。
  \end{enumerate}
...
\section{系统要求}
     ...
     TeX Live 2020~或以上版本。我们推荐您使用最新版本的~TeX Live~发行版（\url{https://www.tug.org/texlive/}），macOS~用户也可选择~MacTeX（\url{https://www.tug.org/mactex/}）。编辑器推荐使用~VS Code~（配合~LaTeX Workshop~插件）、Sublime Text~或~TeXstudio。注意源文件必须以~UTF-8~编码保存。模板默认使用~Times New Roman、Arial、Courier New~以及~Adobe~中文字体；若系统未安装这些字体，模板会自动回退到~TeX Live~自带的~TeX Gyre~系列英文字体和~Fandol~系列中文字体，因此即使不额外安装字体也可正常编译。在满足上述~\TeX~环境条件后，本模板可在~Windows、macOS~和~Linux~平台下使用。
...
本模板依据《天津大学本科生毕业论文说明书模板》和《天津大学关于本科生学位论文统一格式的规定》编写，作者希望能给使用者写作论文带来方便。然而，作者不保证本模板完全符合学校要求，也不对由此带来的风险和损失承担任何责任。
